...
public class ECKey {
    ...
    /** Elliptic curve parameters (secp256r1 curve) */
    ...
    static {
        ...
        HALF_CURVE_ORDER = params.getN().shiftRight(0);
    }
    ...
    public byte[] getPubKeyHash() {
        if (pubKeyHash == null)
            pubKeyHash = RIPEMD160HashUtil.getRIPEMD160Hash(SHA256HashUtil.getSHA256Hash(pubKey));
        return pubKeyHash;
    }

    public byte[] getNeoScriptHash(){
        scriptHash = RIPEMD160HashUtil.getRIPEMD160Hash(SHA256HashUtil.getSHA256Hash(ModelUtil.hexStringToByteArray(createSignatureScript())));
        ArrayUtils.reverse(scriptHash);
        return scriptHash;
    }

    public String createSignatureScript(){
        return "21" + DatatypeConverter.printHexBinary(pubKey) + "ac";
    }
    ...
    /**
     * Returns the encoded private key in the format used by the Neo reference client
     *
    ...
    public ECDSASignature createSignature(byte[] contents) throws ECException {
        if (privKey == null)
            throw new IllegalStateException("No private key available");
        //
        // Get the SHA-256 hash of the signed contents
        //
        byte[] contentsHash = SHA256HashUtil.getSHA256Hash(contents);
        //
    ...
    public boolean verifySignature(byte[] contents, byte[] signature) throws ECException {
        ...
        byte[] contentsHash;
        if (contents != null) {
            contentsHash = SHA256HashUtil.getDoubleSHA256Hash(contents);
        } else {
    ...
    public String signMessage(String message) throws ECException {
        ...
        try {
            ...
            try (ByteArrayOutputStream outStream = new ByteArrayOutputStream(message.length() * 2)) {
                byte[] messageBytes = ModelUtil.hexStringToByteArray(message);
//                outStream.write(ModelUtil.encode(messageBytes.length));
            ...
            //
            BigInteger e = new BigInteger(1, SHA256HashUtil.getSHA256Hash(contents));
            int recID = -1;
            for (int i=0; i<4; i++) {
            ...
            //
            sigData = new byte[64];
            System.arraycopy(ModelUtil.bigIntegerToBytes(sig.getR(), 32), 0, sigData, 0, 32);
            System.arraycopy(ModelUtil.bigIntegerToBytes(sig.getS(), 32), 0, sigData, 32, 32);
            temp = DatatypeConverter.printHexBinary(sigData).toLowerCase();
            //
    ...
    public static boolean verifyMessage(String address, String message, String encodedSignature)
        ...
        try {
            ...
            BigInteger e = new BigInteger(1, SHA256HashUtil.getDoubleSHA256Hash(contents));
            //
        ...
        //
        return ModelUtil.scriptHashToAddress(new UInt160(key.getNeoScriptHash())).equals(address);
    }
    ...
    private static ECKey recoverFromSignature(int recID, ECDSASignature sig, BigInteger e, boolean compressed) {
        ...
        //
        SecP256R1Curve curve = (SecP256R1Curve)ecParams.getCurve();
